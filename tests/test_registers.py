...
@pytest.mark.asyncio
async def test_fetch_registers_modbus_by_id(auth_token, examples):
    host = os.environ["GWTDADOS_HOST"]
    register_modbus_id = examples["valid_register_modbus_id"]
    print("register_modbus_id", register_modbus_id)
    data_registers = await fetch_registers_modbus_by_id(
...
@pytest.mark.asyncio
async def test_fetch_register_dnp_by_sensor(auth_token, examples):
